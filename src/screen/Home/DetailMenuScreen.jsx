...
const DetailMenuScreen = ({ route }) => {
  ...
  useEffect(() => {
    if (isFirstFocus && submitStatus === "succeeded" && paymentUrl) {
      // If it's the first focus and status is succeeded, just reset and mark as not first focus
      dispatch(resetOrderStatus());
      setIsFirstFocus(false);
    } else if (submitStatus === "succeeded" && paymentUrl) {
      // Proceed with navigation only if it's not the first focus or if the status changed after first focus
  ...
  }, [submitStatus, paymentUrl, navigation, dispatch, orderError, isFirstFocus]);
          ...
          <View className="mb-6">
            ...
            <View className="bg-orange-50 p-4 dark:bg-orange-900/30 rounded-2xl mb-4 flex-row items-center justify-between">
              <View className="flex-row items-center">
                ...
                {reviews.length ? (
                  <Text className="text-orange-600 text-xl font-bold">{  item.rating.toFixed(1)}</Text>
                ) : (
                  <Text className="text-gray-600 text-sm">belum ada rating</Text>
                )}
              </View>
